...
export class HelperRouter {
  ...
  resolve(_task: HelperTask): ResolvedHelper | null {
    try {
      ...
      const helperConfig = configManager.getCategory('helper') as Record<string, unknown> | undefined;
      if (helperConfig) {
        ...
        if (providers && currentProviderName && providers[currentProviderName]) {
          ...
          if (perProvider.provider && perProvider.model) {
            try {
              const provider = providerRegistry.get(perProvider.provider);
      ...
      // 2. Global helper
      const globalProvider = configManager.get('helper.globalProvider') as string;
      const globalModel = configManager.get('helper.globalModel') as string;
      if (globalProvider && globalModel) {
        try {
          const provider = providerRegistry.get(globalProvider);
      ...
      // 3. Tool LLM
      const toolProvider = configManager.get('tools.llmProvider') as string;
      const toolModel = configManager.get('tools.llmModel') as string;
      if (toolProvider && toolModel) {
        try {
          const provider = providerRegistry.get(toolProvider);
 ...
 *
 * Singleton pattern, like ToolLLM. Never throws — returns empty string on failure.
 * Tracks token usage separately from the main model.
 */
export class HelperModel {
  private static _instance: HelperModel | undefined;
  private router = new HelperRouter();
  private _usage: HelperUsage = { inputTokens: 0, outputTokens: 0, calls: 0 };

  private constructor() {}

  /** Singleton accessor. */
  static getInstance(): HelperModel {
    if (!HelperModel._instance) {
      HelperModel._instance = new HelperModel();
    }
    return HelperModel._instance;
  }
  ...
  async chat(task: HelperTask, prompt: string, options: HelperChatOptions = {}): Promise<string> {
    ...
    const enabled = configManager.get('helper.enabled') as boolean;
    if (!enabled) {
  ...
  static _reset(): void {
    HelperModel._instance = undefined;
  }
}

/** Singleton instance — import and use everywhere. */
export const helperModel: HelperModel = HelperModel.getInstance();
