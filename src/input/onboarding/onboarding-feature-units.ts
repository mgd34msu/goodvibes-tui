  ...
  },
  {
    ...
    units: [
      ...
      },
      {
        ...
        note: 'On this terminal, turning this on starts a recorder (pw-record, parecord, arecord, ffmpeg or sox — whichever is installed), scores every frame with the pinned classifier, plays a short chime the moment a wake confirms, and shows a persistent "listening" row in the footer for as long as it runs. What follows a wake goes to speech-to-text and lands in the composer, or is sent straight away if you turn voice.wake.autoSubmit on. The model it scores with is already here — installing GoodVibes downloads and checksum-verifies it, and a running daemon retries at boot if the install could not reach the network — so turning this on normally needs no setup step. Two things it will NOT do on its own: download anything at the moment you turn it on (a host whose model is missing or fails verification says which and names /voice wake setup, instead of pretending to listen) and run with voice.wake.vadThreshold above 0, which refuses to start because no voice-activity model is pinned to screen frames with.',
      },
    ],
  },
  {
