  ...
  },
  {
    ...
    units: [
      ...
      },
      {
        ...
        note: 'On this terminal, turning this on starts a recorder (pw-record, parecord, arecord, ffmpeg or sox — whichever is installed), scores every frame with the pinned classifier, plays a short chime the moment a wake confirms, and shows a persistent "listening" row in the footer for as long as it runs. What follows a wake goes to speech-to-text and lands in the composer, or is sent straight away if you turn voice.wake.autoSubmit on. Two things it will NOT do on its own: download the models (run /voice wake setup — until then it says so instead of pretending to listen) and run with voice.wake.vadThreshold above 0, which refuses to start because no voice-activity model is pinned to screen frames with.',
      },
    ],
  },
  {
