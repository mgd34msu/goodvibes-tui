...
describe('operator surfaces gate', () => {
  ...
  test('built-in strategic operator panels are registered on the active runtime surface', () => {
    ...
    // W6.1 (the purge) — group B: policy/hooks/security/marketplace/knowledge
    // migrated to config-modals. They are no longer registered panels; each
    // redirects to its modal via PanelManager.registerModalRedirect (registered
    // by registerEcosystemModalRedirects in registerOperationsPanels).
