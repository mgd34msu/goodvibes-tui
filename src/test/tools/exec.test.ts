...
describe('exec tool — simple commands', () => {
  test('runs a simple echo command', async () => {
    const result = await execTool.execute({ commands: [{ cmd: 'echo hello' }] });
  ...
  test('captures stdout', async () => {
    const result = await execTool.execute({ commands: [{ cmd: 'printf "line1\nline2"' }] });
    expect(result.success).toBe(true);
    const out = parseOutput(result.output);
  ...
  test('captures stderr', async () => {
    const result = await execTool.execute({ commands: [{ cmd: 'echo err >&2' }] });
    expect(result.success).toBe(true);
    const out = parseOutput(result.output);
  ...
  test('exit code 0 for success', async () => {
    const result = await execTool.execute({ commands: [{ cmd: 'true' }] });
    expect(result.success).toBe(true);
    const out = parseOutput(result.output);
    expect(out.exit_code).toBe(0);
  ...
  test('exit code non-zero for failure', async () => {
    const result = await execTool.execute({ commands: [{ cmd: 'false' }] });
...
describe('exec tool — background mode', () => {
  ...
  test('bg_stop removes the process', async () => {
    ...
    const stopResult = await execTool.execute({
...
describe('exec tool — file_ops', () => {
  ...
  test('delete: deletes a file before command runs', async () => {
    // delete is restricted to project root — use a project-relative path
    const projectTmp = join(process.cwd(), '.goodvibes', 'test-tmp');
    mkdirSync(projectTmp, { recursive: true });
    const target = join(projectTmp, 'todelete.txt');
