...
describe('a desktop notification carries no chain identifier', () => {
  ...
  test('the workstream lines still say what happened — this is not a suppression fix', () => {
    const bodies = notifyCalls().join('\n');
    expect(bodies).toContain('cancelled');
    expect(bodies).toContain('turn budget');
    expect(bodies).toContain('could not be finished');
  });
});
