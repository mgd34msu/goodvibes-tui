/**
 * GC-ORCH-015: Unresolved tool result reconciliation tests.
 *
 * Covers:
 ...
 * - Legacy compatibility mode (flag disabled → warning only)
 *
...
describe('Orchestrator GC-ORCH-015 reconciliation', () => {
  let bus: EventBus;
  ...
  test('emits turn:tool-reconciliation for malformed stopReason=tool_use with no tool calls', async () => {
    const { orch } = await buildOrchestrator();

    // Collect reconciliation events
    const reconEvents: ReconciliationEvent[] = [];
    bus.on('turn:tool-reconciliation', (evt) => reconEvents.push(evt));
    ...
    bus.emit('turn:tool-reconciliation', {
      count: 0,
      callIds: [],
      toolNames: [],
  ...
  test('reconcileUnresolvedToolCalls is a no-op when _pendingToolCalls is empty', async () => {
    ...
    bus.on('turn:tool-reconciliation', () => messages.push('reconciled'));
  ...
  test('reconcileUnresolvedToolCalls emits turn:tool-result for each synthetic result', async () => {
    const { orch } = await buildOrchestrator();
    ...
    bus.on('turn:tool-result', (data) => toolResults.push(data));
  ...
  test('isReconciliationEnabled returns false when flag is disabled in manager', async () => {
    ...
    const { ConversationManager } = await import('../../core/conversation.ts');
    const { PermissionManager } = await import('../../permissions/manager.ts');
    const { FeatureFlagManager } = await import('../../runtime/feature-flags/manager.ts');

    const cm = new ConversationManager(() => 80);
    const pm = new PermissionManager(bus);
    const flagManager = new FeatureFlagManager();
    flagManager.disable('gc-orch-015-tool-result-reconciliation');

    const orch = new Orchestrator(bus, cm, () => 24, () => {}, toolRegistry, pm, () => '', null, flagManager);
    const enabled = (orch as unknown as { isReconciliationEnabled: () => boolean }).isReconciliationEnabled();
    expect(enabled).toBe(false);
  });

  // ---------------------------------------------------------------------------
  // Compatibility mode: flag disabled
  // ---------------------------------------------------------------------------

  test('reconcileUnresolvedToolCalls does NOT emit event when flag is disabled (legacy mode)', async () => {
    const { Orchestrator } = await import('../../core/orchestrator.ts');
    const { ConversationManager } = await import('../../core/conversation.ts');
    const { PermissionManager } = await import('../../permissions/manager.ts');
    const { FeatureFlagManager } = await import('../../runtime/feature-flags/manager.ts');
    ...
    const danglingCall: ToolCall = { id: 'legacy-1', name: 'read', arguments: {} };
    (orch as unknown as { _pendingToolCalls: ToolCall[] })._pendingToolCalls = [danglingCall];
    ...
    // In legacy mode: no event, no synthetic injection
    expect(reconEvents).toHaveLength(0);
    // Pending state is cleared even in legacy mode
