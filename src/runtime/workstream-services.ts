...
export interface WorkstreamCommandService {
  ...
  /** `isolation` omitted ⇒ the engine's own default ('shared'). See CreateWorkstreamInput.isolation (SDK). */
  proposeDraft(task: string, isolation?: WorkstreamIsolation): WorkstreamDraft;
  getDraft(id: string): WorkstreamDraft | undefined;
  listDrafts(): WorkstreamDraft[];
  /** Re-derive a held draft's spec from a new task string. Clears any prior approval — an edit must be re-approved. `isolation` omitted ⇒ keeps the draft's current choice (an edit that only changes the task text must not silently reset isolation back to shared). */
  editDraft(id: string, task: string, isolation?: WorkstreamIsolation): WorkstreamDraft | undefined;
