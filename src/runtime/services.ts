...
export interface RuntimeServicesOptions {
  ...
  /** Daemon state root (`--daemon-home`); absent ⇒ `<homeDirectory>/.goodvibes/daemon`.
   * Threaded into SecretsManager so the override MOVES the credential store —
   * without it an "isolated" daemon read the real home's secrets. One name for
   * one thing; resolveGoodVibesHomeOwnership is the single reader. */
