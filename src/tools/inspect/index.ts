...
function summarizeInspectValue(mode: InspectMode, value: unknown): unknown {
...
function serializeInspectOutput(value: unknown, format: string, mode: InspectMode): string {
...
function createInspectSuccess(output: unknown, format: string, mode: InspectMode): InspectToolResult {
...
function runFileInspection<T>(
  ...
  return createInspectSuccess(inspect(file.content, inputFile!), format, mode as InspectMode);
}
    ...
    try {
      switch (input.mode as InspectMode) {
        case 'project': {
          ...
          return createInspectSuccess(info, format, input.mode);
        }
        ...
        case 'database': {
          ...
          return createInspectSuccess(dbInfo, format, input.mode);
        }
        ...
        case 'layout': {
          ...
          return createInspectSuccess(layoutInfo, format, input.mode);
        }
        ...
        case 'api_spec': {
          ...
          return createInspectSuccess(spec, format, input.mode);
        }
        ...
        case 'api_validate': {
          ...
          return createInspectSuccess(validationResult, format, input.mode);
        }

        case 'api_sync': {
          const framework: ApiFramework = (input.framework ?? 'auto') as ApiFramework;
          const syncResult = await inspectApiSync(projectRoot, framework);
          return createInspectSuccess(syncResult, format, input.mode);
        }
        ...
        case 'scaffold': {
          ...
          return createInspectSuccess(plan, format, input.mode);
        }
        ...
        case 'component_state': {
          ...
          return createInspectSuccess(result, format, input.mode);
        }
