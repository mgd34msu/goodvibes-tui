...
const ANALYZE_SUMMARY_SAMPLE_LIMIT = 5;

function sampleArray<T>(value: unknown, limit = ANALYZE_SUMMARY_SAMPLE_LIMIT): T[] {
  return Array.isArray(value) ? (value as T[]).slice(0, limit) : [];
}

function summarizeAnalyzeResult(mode: AnalyzeInput['mode'], result: Record<string, unknown>): Record<string, unknown> {
  switch (mode) {
    case 'dependencies':
      return {
        mode,
...
function applyAnalyzeTokenBudget(output: string, maxTokens: number | undefined): string {
...
export const analyzeTool: Tool = {
  ...
  async execute(args: Record<string, unknown>): Promise<{ success: boolean; output?: string; error?: string }> {
    try {
      ...
      const fingerprinted = appendSchemaFingerprint(result, 'analyze', input.mode);
