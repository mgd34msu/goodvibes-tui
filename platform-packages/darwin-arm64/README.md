# @pellux/goodvibes-tui-darwin-arm64

Prebuilt `goodvibes` binary for **darwin-arm64**, alongside the matching
`sqlite-vec` native addon (`vec0.dylib`) that the terminal's own `/recall`
