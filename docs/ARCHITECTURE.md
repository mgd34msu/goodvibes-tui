...
A typed publish-subscribe bus that decouples all major subsystems. Key events:

- `input:submit` — user sends a message
- `input:interrupt` — user cancels generation (Ctrl+C)
- `command:execute` — a slash command runs
- `command:model-changed` — model or provider switches
- `agent:spawn` / `agent:complete` / `agent:error` — agent lifecycle
- `permission:request` / `permission:response` — tool permission prompts
- `render:request` — any subsystem requests a redraw

### Orchestrator (`src/core/orchestrator.ts`)

The main conversation loop. On each turn:

1. Receives `input:submit` from the EventBus
...
6. On completion: triggers HookDispatcher post-turn hooks, checks auto-compact threshold, emits `render:request`
...
3. Reports progress via the EventBus
