...
Open, place, resize, or list panels.

```
/panel              # open the panel sidebar
/panel list         # list available panels
/panel open <id>    # open a panel by ID in the focused pane
/panel open <id> bottom  # open a panel directly in the bottom pane
/panel close <id>   # close a panel
/panel toggle       # toggle the panel sidebar
/panel move other   # move the active panel to the other pane
/panel focus toggle # switch focused pane
/panel split        # toggle bottom pane visibility
/panel width left   # widen the conversation area
/panel width right  # widen the panel area
/panel height up    # move the horizontal split upward
/panel height down  # move the horizontal split downward
```
