# Decision: the session-spine mode branch (`syncSessionSpineToHostStatus`) is permanent design, not a staging escape hatch, and embedded/in-process operation stays
...
The owner's D7a decision shipped detached-daemon-by-default with `daemon.embedInProcess` as an
explicit, documented, opt-in topology (default `false`, "NOT RECOMMENDED" for the reasons in its
own description, since it couples the daemon's lifetime to one surface, but a real, supported
mode). Nothing in that prior cleanup pass or any earlier one retired it. The ruling for this
brief, which the rest of this document treats as its premise, is that embedded/in-process daemon
operation stays. It is a permanent topology, not a staging vestige left over from the earlier
spine conversion.
...
The brief's own investigation (binding_architectural_findings, item "REMOVAL 2") had already
flagged the caveat that makes this ambiguous. `embedded` and `external` are **different daemon
topologies**, not two ends of one staged rollout. `embedded` means this TUI process *is* the
daemon (there is nothing to mirror; the local broker already is the daemon's broker). `external`
means this TUI adopted a separate, detached daemon process and mirrors its own session identity
into it. Collapsing the branch to "always mirror" would be wrong for embedded. There is no second
process to mirror to.
...
4. **The parallel-write mirror stays a mirror, not authoritative, by design, not
   oversight.** `bootstrap-core.ts:742-745` registers the local session with the spine
   ...
   escape hatch," is not implied by anything the owner ruled, and is explicitly out of this
...
2. **Scheduled Removal 2, as literally described in the prior cleanup pass's ledger ("retire the staging
   ...
   silently change a daemon topology") and the owner's no-deferral rule's real-reason bar for
...
- **Make the spine authoritative and drop the local-broker mirror.** Rejected as out of
  ...
  retiring a staging escape hatch, was never ruled by the owner, and the prior cleanup pass's own
...
> architectural change than "retire a staging escape hatch," is not implied by anything the owner
...
> escape hatch, was never ruled by the owner, and the prior cleanup pass's own risk note explicitly
...
The owner ruled, later the same day, that this residual is removed outright. `daemon.embedInProcess`
and its schema row are deleted as a product-facing setting. The SDK's embed capability itself is
not deleted; it stays as composition machinery for an embedder or a test harness to construct a
`DaemonServer`/`HttpListener` factory pair directly and call `startHostServices` without
`adoptOnly`, exactly the shape `src/test/runtime/bootstrap-services.test.ts` exercises against the
SDK's exported function today. What is gone is the idea that a shipped product exposes a setting
that could ever select it.
...
in-process HTTP listener as `'embedded'`, and that fact did not change and is not part of what the
owner ruled on here):
...
- `src/runtime/bootstrap-core.ts`'s `sessionSpine`, `sessionUnionCache`, and the heartbeat call are
  ...
  fact about this app's `adoptOnly` stance, are rewritten to state the current, stronger reason.
  No product-facing setting exists that could select it at all. Nothing was deleted here. The SDK's
