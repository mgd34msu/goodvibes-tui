...
# and published to goodvibes.sh by that repository's release workflow — it
# lives with the terminal app because that repository's release lane is the
# one that publishes it. It is the ONLY copy: a second one in another
# repository is exactly how two installers drift apart. The `/update` command
# re-runs the same download-verify-swap logic; keep the two in lockstep.
